    ...
    <MainLayout>
      <div className="flex-1 flex flex-col md:flex-row h-[calc(100vh-4rem)] md:h-screen">
        <div className={`flex flex-col z-20 will-change-transform transition-all ${isMemoBrowserOpen ? '' : 'absolute h-full -translate-x-full opacity-0 pointer-events-none'}`}>
          <div
            ref={memoBrowserRef}
            className="flex-1 h-full border-r-3 border-border/30 flex-col relative transition-all duration-50 ease-linear hidden md:flex"
            style={{ width: `${memoBrowserWidth}px`, minWidth: '400px', maxWidth: '700px' }}
          >
            <MemoBrowserWrapper />
            <div
              className={`group absolute -right-1.25 top-0 bottom-0 w-1.5 transition-colors duration-150 cursor-col-resize`}
